...
//   t_append_end/t_end. GPU_EDGE_LOD_DONE is a separate stable event
//   (lod_shared.lod_done[fc]), not the timing t_end.
...
enum gpu_edge
{
  ...
  GPU_EDGE_LOD_DONE,    // compute -> compress: LOD chunks in pool
                        // (bound to lod_shared.lod_done[fc]; multiscale only)
