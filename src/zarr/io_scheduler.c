...
struct io_scheduler
{
  struct platform_thread* dispatcher;
  ...
  uint64_t in_flight;     // dispatched or running and not yet finished
  uint64_t backend_in_flight;
  uint64_t dispatched;
  ...
  struct io_scheduler_counters counters;
...
// measured from.
static void
writes_on_file_changed(struct io_scheduler* q, int64_t delta, int64_t now)
{
  q->files_with_writes = (uint64_t)((int64_t)q->files_with_writes + delta);
  io_scheduler_counters_files_waiting(&q->counters, q->files_with_writes, now);
}
...
// behind it.
static uint64_t
file_next_ready(struct io_scheduler* q, const struct file_pending* f)
{
...
{
  if (q->dispatched >= q->max_dispatched)
    return NO_SEQ;
...
static void
retire_job(struct io_scheduler* q, struct io_completion c, int64_t finished_ns)
{
  void* owned = NULL;
  void (*owned_free)(void*) = NULL;
  void* completion_ctx = NULL;
  void (*completed)(void*, const struct io_completion*) = NULL;
  ...
  owned = job.original.owned;
  owned_free = job.original.owned_free;
  completion_ctx = job.original.completion_ctx;
  completed = job.original.completed;
  if (c.nbytes <= UINT64_MAX - job.completed_bytes)
    c.nbytes += job.completed_bytes;
  else {
    c.nbytes = job.completed_bytes;
    c.status = IO_FAILED;
  }
  ...
  while (q->tail < q->head && job_at(q, q->tail)->state == IO_JOB_RETIRED)
    q->tail++;
  q->pending_bytes -= job.original.nbytes;
  q->in_flight--;
  q->backend_in_flight--;
  io_scheduler_counters_in_flight(
    &q->counters, q->backend_in_flight, finished_ns);
  file_request_retired(q, c.seq, &job.original, finished_ns);
  io_scheduler_counters_finished(
    &q->counters, &job.original, job.post_ns, job.started_ns, finished_ns);
...
static int
retry_partial(struct io_scheduler* q,
              ...
              int64_t now)
{
  ...
  struct io_job* slot = job_at(q, completion->seq);
  CHECK(Unlock, slot->seq == completion->seq && slot->state == IO_JOB_RUNNING);
  if (completion->status != IO_OK && completion->status != IO_PARTIAL)
    goto Unlock;

  const uint64_t asked = slot->req.nbytes;
  if (completion->nbytes == asked) {
    completion->status = IO_OK;
    goto Unlock;
  }
  if (slot->req.op != IO_OP_WRITE || completion->nbytes == 0 ||
      completion->nbytes > asked) {
    completion->nbytes = 0;
    completion->status = IO_FAILED;
    goto Unlock;
  }

  slot->completed_bytes += completion->nbytes;
  slot->req = io_write_remaining(&slot->req, completion->nbytes);
  slot->state = IO_JOB_WAITING;
  q->jobs_waiting++;
  q->in_flight--;
  q->backend_in_flight--;
  io_scheduler_counters_in_flight(&q->counters, q->backend_in_flight, now);
...
static int
requeue_job(struct io_scheduler* q, uint64_t seq, int64_t now)
{
  int requeued = 0;
...
static void
executor_started(void* arg, uint64_t seq)
{
  struct io_scheduler* q = (struct io_scheduler*)arg;
  platform_mutex_lock(q->mutex);
  struct io_job* slot = job_at(q, seq);
  CHECK(Unlock, slot->seq == seq && slot->state == IO_JOB_DISPATCHED);
  slot->state = IO_JOB_RUNNING;
  const int64_t now = platform_monotonic_ns();
  if (slot->req.nbytes > 0 && slot->started_ns == 0)
    slot->started_ns = now;
  q->dispatched--;
  q->jobs_waiting--;
  q->backend_in_flight++;
...
static void
executor_finished(void* arg, struct io_completion completion)
{
  struct io_scheduler* q = (struct io_scheduler*)arg;
  const int64_t now = platform_monotonic_ns();
  if (retry_partial(q, &completion, now))
    return;
  retire_job(q, completion, now);
}

static void
executor_busy(void* arg, uint64_t seq)
{
  struct io_scheduler* q = (struct io_scheduler*)arg;
  const int64_t now = platform_monotonic_ns();
  ...
  log_error("io_scheduler: a refused request named a file that was reopened");
  retire_job(q,
...
dispatcher_main(void* arg)
{
  ...
  for (;;) {
    ...
    // The scheduler slot outlives execution, so the queue only needs a
    // pointer to the request selected here.
    const struct io_request* req = &slot->req;
    platform_mutex_unlock(q->mutex);
    if (io_queue_post(q->queue, req, seq))
      executor_busy(q, seq);
  }
}

static void
queue_free(struct io_scheduler* q)
{
  free(q->ring);
  free(q->files);
  free(q->order);
                    ...
                    struct io_scheduler_limits limits)
{
  ...
  q->max_dispatched = limits.workers ? limits.workers : 1;
  ...
  q->queue = io_queue_create(backend,
  ...
  q->dispatcher = platform_thread_start(dispatcher_main, q);
  if (!q->dispatcher) {
    io_queue_destroy(q->queue);
    q->queue = NULL;
...
io_scheduler_destroy(struct io_scheduler* q)
{
  ...
  if (q->dispatcher)
    platform_thread_join(q->dispatcher);
  io_queue_destroy(q->queue);
  q->queue = NULL;
...
{
  struct io_scheduler* mq = (struct io_scheduler*)q;
...
static int
names_a_closing_file(struct io_scheduler* q, const struct io_request* req)
{
  if (req->file.generation == 0)
    return 0;
...
int
io_scheduler_reserve(struct io_scheduler* q, struct io_request req)
{
  ...
  if (q->shutdown) {
    ...
    log_error("io_scheduler: refused a request naming a file that is closing");
    refused = 1;
...
void
io_scheduler_commit(struct io_scheduler* q, struct io_request req)
{
  platform_mutex_lock(q->mutex);
...
void
io_scheduler_release(struct io_scheduler* q, uint64_t nbytes)
{
  platform_mutex_lock(q->mutex);
  CHECK(Unlock, q->reserved_requests > 0);
...
void
io_scheduler_complete(struct io_scheduler* q, struct io_completion c)
{
  io_queue_complete(q->queue, c);
}

int
io_scheduler_post(struct io_scheduler* q, struct io_request req)
{
  if (io_scheduler_reserve(q, req))
    return 1;
  io_scheduler_commit(q, req);
  return 0;
}
...
void
io_scheduler_get_stats(const struct io_scheduler* q, struct io_queue_stats* out)
{
  struct io_scheduler* mq = (struct io_scheduler*)q;
  platform_mutex_lock(mq->mutex);
  io_scheduler_counters_read(&mq->counters, out, platform_monotonic_ns());
  platform_mutex_unlock(mq->mutex);
}
