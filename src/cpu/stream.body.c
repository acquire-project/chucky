...
Done:
  ...
  if (shard_sink_drain(v->sink))
    failed = 1;
  if (v->config->prepare_shards && shard_sink_cancel_prepared(v->sink))
    failed = 1;
...
{
  int failed = v->config->prepare_shards && shard_sink_cancel_prepared(v->sink);
