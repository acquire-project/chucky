...
// Spin budget before falling back to condvar wait. ~10µs target on modern
// hardware: long enough to absorb back-to-back dispatches without a syscall,
// short enough that idle pools sleep quickly.
//
// PAUSE/YIELD on x86 retires in ~5–25 cycles depending on uarch (Skylake+
// can stall up to ~140); we conservatively assume ~10 cycles to convert µs
// → spin count for a 1 GHz CPU. Real chips are 3–5×, so the actual spin time
// is closer to 2–10 µs across the supported range — well below a syscall.
#define THREADPOOL_SPIN_ITERS 10000u

// Cache-line size used for atomic alignment. 64 bytes covers x86 and AArch64
// well; the 128-byte Intel spatial-prefetcher edge case is not worth the
// extra padding for a pool of this size.
...
// Task fn used internally by all dispatch entry points. The dispatcher
// installs a (fn, ctx) pair, bumps `epoch`, and broadcasts.
...
worker_main(void* arg)
{
  ...
  for (;;) {
    ...
    // Phase B: condvar sleep if still nothing.
    if (epoch == last_epoch &&
        !atomic_load_explicit(&p->stop, memory_order_acquire)) {
...
// Run fn(tid, ctx) on every participant (caller as tid 0, workers as tids
// 1..nworkers), then return when all have finished.
static void
dispatch(struct threadpool* p, pool_task_fn fn, void* ctx)
{
  ...
  // Hold the mutex across epoch++ + broadcast: workers that are about to call
  // cond_wait must observe either the new epoch (and skip the wait) or the
  // broadcast (and wake from the wait). Without the lock, the worker can
  // observe the old epoch, then enter cond_wait, after our broadcast — and
  // miss it.
  platform_mutex_lock(p->mu);
  ...
  // Caller does its share of the work as tid 0.
  fn(0, ctx);

  // Spin-join — workers finishing their slice in parallel; brief.
...
threadpool_new(int nthreads)
{
  ...
  for (int i = 0; i < nthreads; ++i) {
    p->workers[i].pool = p;
    p->workers[i].tid = i + 1; // caller is tid 0
