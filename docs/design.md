...
| `buffer_capacity_bytes` | > 0 | H2D staging buffer size, rounded up to a 4096-byte page and doubled internally. Also the dispatch size: nothing reaches the device until it fills or the caller flushes, so a larger value raises throughput and lets an append wait longer. A dispatch never exceeds one batch's remaining bytes (one epoch's, when multiscale), so raising this past that costs pinned memory without grouping any further |
