...
def record_build():
    info = dict(source_commit=HEAD, patch_sha256=check_source(), built_utc=now(),
                ...
                nvcomp_library=next(line.split('=', 1)[1] for line in (REPO / 'build/CMakeCache.txt').read_text().splitlines() if line.startswith('NVCOMP_LIBRARY:')),
...
def run():
    ...
    if len(names) != 1 or names[0].strip() != 'NVIDIA GeForce RTX 5070 Laptop GPU':
        raise RuntimeError(f'Expected one Auk RTX 5070 Laptop GPU: {names}')
    raw = ROOT / 'raw'
...
def summarize():
    ...
    for row in summaries:
        ...
        row['auk_frontier_in_subset'] = not any(dominated(r, 'throughput_median_gibs', 'compression_fold') for r in candidates)
