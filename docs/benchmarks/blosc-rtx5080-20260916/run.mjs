// Reproduce the 200-configuration matrix and retain failed executions.
...
const original = JSON.parse(fs.readFileSync(path.join(root, 'docs/benchmarks/blosc-rtx5080-20260905/provenance.json')));
const rawPath = path.join(here, 'raw-results.jsonl');
const provenancePath = path.join(here, 'provenance.json');
const hash = p => crypto.createHash('sha256').update(fs.readFileSync(p)).digest('hex');
if (hash(exe) !== original.executable_sha256) throw new Error('Expected the validated September 5 benchmark binary');
...
const provenance = {
  ...
  scenario: 'orca2_single', source_commit: original.source_commit,
  collection_checkout_commit: command('git', ['rev-parse', 'HEAD']),
  tracked_source_diff: original.tracked_source_diff,
...
for (let pass = 0; pass < 4; ++pass) for (const config of shuffle(configs)) {
  ...
  const output = await run(args).catch(error => ({
    code: null, timed_out: false, stdout: '', stderr: String(error),
  }));
  ...
  if (validation_error) console.warn(`Recorded failure for ${config.fill}/${config.chunk_kib}/${config.codec}/${config.block_kib}: ${validation_error}`);
}
provenance.finish_utc = new Date().toISOString();
provenance.gpu_after = gpu();
provenance.complete = provenance.runs === 800;
...
save();
console.log(`Complete; ${provenance.failures.length} failed executions retained in raw-results.jsonl.gz`);
