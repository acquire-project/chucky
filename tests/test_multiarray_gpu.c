...
test_basic_two_array(void)
{
  ...
  // Array 0: 3D 4x4x6, chunk 2x2x3, cps 2x1x1
...
test_content_isolation(void)
{
  ...
  struct multiarray_tile_stream_gpu* ms =
    multiarray_tile_stream_gpu_create(2, configs, sinks_arr, 0);
  CHECK(Fail, ms);
...
// ---- Test: cross-validate multiarray (1 array) vs single-array GPU ----
...
test_cross_validate_single_array(void)
{
  ...
  struct tile_stream_gpu* ref = NULL;
  ...
  ref = tile_stream_gpu_create(&config_r, &sink_ref.base);
  CHECK(Fail, ref);
  {
    struct writer* w = tile_stream_gpu_writer(ref);
...
test_metrics_enabled(void)
{
  ...
  struct multiarray_tile_stream_gpu* ms =
    multiarray_tile_stream_gpu_create(1, configs, sinks, 1);
  CHECK(Fail, ms);
  ...
  struct stream_metrics m = multiarray_tile_stream_gpu_get_metrics(ms);
  CHECK(Fail, m.memcpy.count > 0);
  CHECK(Fail, m.sink.count > 0);
  log_info("  memcpy: %lu calls, sink: %lu calls",
           (unsigned long)m.memcpy.count,
           (unsigned long)m.sink.count);
